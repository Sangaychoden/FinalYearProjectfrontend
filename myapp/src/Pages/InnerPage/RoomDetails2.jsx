...
const RoomDetails2 = () => {
  ...
  // booking-level base adults and children inputs
  const [adult, setAdult] = useState(1);
  const [children, setChildren] = useState(0);
  ...
  // per-room occupancy array (length == room)
  const [occupancyPerRoom, setOccupancyPerRoom] = useState(
    Array.from({ length: 1 }, () => "double")
  );
  ...
  const getChildUnitPriceForAge = (ageGroup, occ) => {
    ...
    if (ageGroup === "6-11") return childPolicy?.age6to11?.[mealPlan] ?? 0;
    if (ageGroup === "12+") {
      // 12+ treated as adult (use base occupancy price)
      return getBasePriceForOccupancy(occ);
    }
    return 0;
  };

  // ---------- NEW: derived counts that treat 12+ as adult ----------
  const num12Plus = childrenAges.filter((a) => a === "12+").length;
  const displayAdults = adult + num12Plus; // shown / sent to backend as adults
  const displayChildren = Math.max(0, children - num12Plus); // shown / sent to backend as children

  // filtered children ages (exclude 12+ since they're now adults)
  const filteredChildrenAges = childrenAges.filter((a) => a !== "12+");
  // -----------------------------------------------------------------

  // compute child cost total per night across remaining children
  const getChildCostPerNight = () => {
    if (!roomData?.childPolicy) return 0;
    if (filteredChildrenAges.length === 0) return 0;

    // pick default occupancy for pricing for 12+ conversion uses; choose most common occupancy
    const occCounts = occupancyPerRoom.reduce((acc, o) => {
      acc[o] = (acc[o] || 0) + 1;
      return acc;
    }, {});
    ...
    let sum = 0;
    filteredChildrenAges.forEach((age) => {
      sum += getChildUnitPriceForAge(age, defaultOcc);
    });
    return sum;
  };

  // total calculation (per-night base rooms + child costs + extra bed)
  const calcTotal = () => {
    const nights = calcNights();
    if (!nights) return "0.00";
    ...
    // children cost per night (now excludes 12+)
    const childCostPerNight = getChildCostPerNight();
    ...
    // meals cost is handled in booking details (not charged here)
    const mealsPerNightTotal = 0;
  ...
  const checkRoomAvailability = async () => {
    ...
    // ---------- USE displayAdults & displayChildren in availability query ----------
    const url =
      `${API_URL}/rooms/${roomType}/availability` +
      `?checkIn=${fmt(checkIn)}` +
      `&checkOut=${fmt(checkOut)}` +
...
Swal.fire({
  ...
  color: "#fff",
        ...
        if (result.isConfirmed) {
          ...
          const roomSelection = Object.keys(occCounts).map((occ) => {
            const count = occCounts[occ];
            // approximate adults for this occ group
            const approxAdults = Math.max(1, Math.round((displayAdults * (count / room)) || 1));
            return {
              roomType,
              roomsRequested: count,
              occupancyType: occ,
              mealPlan,
              adults: approxAdults,
              childrenAges: filteredChildrenAges, // send remaining children only
              extraBed: extraBed && (mealPlan === "map" ? true : occ === "double"),
            };
          });
          ...
          navigate("/booking_details", {
            state: {
              ...
              checkOut,
              room, // number of rooms
              adult: displayAdults, // send converted adults
              children: displayChildren, // send remaining children
  ...
  <ul className="text-md space-y-3 leading-relaxed">
    ...
    <li>
      ...
      <span className="text-[#666666]">
        ...
        <br />
        <strong>MAP: The extra room option is available for single- and double-occupancy rooms.</strong>
      </span>
    </li>
          ...
          <div className="p-6 h-full flex flex-col relative">
            ...
            {!extraOptionsVisible && (
              <>
                ...
                <div ref={guestRef} className="mb-4 relative">
                  ...
                  {/* ---------- SHOW derived counts here (12+ -> adult immediately) ---------- */}
                  <div
                    className="border border-white px-3 py-2 text-white flex justify-between cursor-pointer"
                    onClick={() => setGuestOpen(!guestOpen)}
                  >
                    {displayAdults} Adult{displayAdults > 1 ? "s" : ""}, {displayChildren} Child{displayChildren !== 1 ? "ren" : ""}
                    <BiChevronDown />
                  </div>
                  {/* --------------------------------------------------------------------------- */}
            ...
            {extraOptionsVisible && children > 0 && (
              <div className="-mb-6 mt-8">
                ...
                {/* NOTE: when a child is changed to 12+, displayAdults updates immediately because displayAdults derives from childrenAges */}
              </div>
            )}
